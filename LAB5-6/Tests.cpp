...
void Tests::testAdminAdd() {
    ...
    assert( this->movieController.getMovieRepository().getMovies().getLength() == 1 );

    this->movieController.add("Medium", "Action", 2000, 15, "https://yes");
    assert( this->movieController.getMovieRepository().getMovies().getLength() == 2 );

    this->movieController.add("A", "Action", 300, 45, "https://DA");
    assert( this->movieController.getMovieRepository().getMovies().getLength() == 3 );

    this->movieController.add("B", "Action", 1950, 65, "https://NU");
    assert( this->movieController.getMovieRepository().getMovies().getLength() == 4 );

    this->movieController.add("C", "Action", 1980, 75, "https://GIGI");
    assert( this->movieController.getMovieRepository().getMovies().getLength() == 5 );
...
void Tests::testUserAdd() {
    ...
    assert( this->movieController.getWatchListRepository().getWatchList().getLength() == 1);

    this->movieController.getWatchListRepository().add(WatchListItem("B"));
    assert( this->movieController.getWatchListRepository().getWatchList().getLength() == 2);

    this->movieController.getWatchListRepository().add(WatchListItem("C"));
    assert( this->movieController.getWatchListRepository().getWatchList().getLength() == 3);

    this->movieController.getWatchListRepository().add(WatchListItem("D"));
    assert( this->movieController.getWatchListRepository().getWatchList().getLength() == 4);

    this->movieController.getWatchListRepository().add(WatchListItem("E"));
    assert( this->movieController.getWatchListRepository().getWatchList().getLength() == 5);
...
void Tests::testUserDel() {
    ...
    assert( this->movieController.getWatchListRepository().getWatchList().getLength() == 0);
